...
export function MemberName({ member, name }: { member?: Member; name?: string }) {
  ...
  return <span className="member-nameplate" data-rank={member?.gameProfile?.rank}>
    {member?.gameProfile?.rank === "R5" && <svg className="leader-crown" viewBox="0 0 24 24" role="img" aria-label={t("Alliance leader")} focusable="false">
      <title>{t("Alliance leader")} · R5</title>
