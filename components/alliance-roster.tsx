...
export function AllianceRoster({ canManage = false, state, onOpenMember, onMergeMember, onDeleteMember, onSaved }: { canManage?: boolean; state: TrackerState; onSaved: (state: TrackerState) => void; onOpenMember: (id: string) => void; onMergeMember: (id: string) => void; onDeleteMember: (id: string) => void }) {
  ...
  return <div className="page-stack alliance-roster-page">
    ...
    <section className="alliance-roster-directory" aria-label="Alliance members">
      <header className="alliance-roster-heading"><div className="alliance-header-summary"><h2>{state.alliance.name || "My alliance"} <span>· {active.length} members</span></h2>{leader && <p>Leader: <MemberName member={leader} /><b className="alliance-rank" data-rank="R5">R5</b></p>}</div><span className="alliance-source-date">{lastUpdated ? `Last updated ${new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" }).format(new Date(`${lastUpdated}T00:00:00Z`))}` : "No profile updates"}</span></header>
