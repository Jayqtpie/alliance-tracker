  ...
  return <div className="page-stack alliance-roster-page">
    ...
    <section className="alliance-roster-card" aria-label="Alliance members">
      ...
      <div className="alliance-roster-toolbar">
        <div className="search-box"><Search size={16} /><input aria-label="Search roster" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a commanderâ€¦" />{query && <button className="search-clear" aria-label="Clear roster search" onClick={() => setQuery("")}><X size={14} /></button>}</div>
        ...
        <select aria-label="Sort roster" value={sort} onChange={(event) => setSort(event.target.value)}><option value="heroPower">Hero power â†“</option><option value="kills">Kills â†“</option><option value="name">Name Aâ€“Z</option></select>
      </div>
      ...
      <ol id={rosterListId} className="alliance-roster-list">{filtered.map(({ member, position }) => <li key={member.id}>
        <button className="alliance-roster-row" onClick={() => onOpenMember(member.id)} aria-label={`View ${member.canonicalName}, hero power ${member.gameProfile?.heroPowerDisplay ?? "unavailable"}, kills ${member.gameProfile?.killsDisplay ?? "unavailable"}`}>
          ...
          <span className="alliance-stat"><strong>{member.gameProfile?.heroPowerDisplay ?? "â€”"}</strong>{member.gameProfile?.heroPowerLegacy && <small className="alliance-legacy" title="LWServers marks this hero power as legacy data">Legacy</small>}</span>
          <span className="alliance-stat"><strong>{member.gameProfile?.killsDisplay ?? "â€”"}</strong></span>
        </button>
      </li>)}</ol></div>
      ...
      <footer className="alliance-roster-foot"><span aria-live="polite">{filtered.length} {filter === "active" ? "members" : "previous records"}{rankFilter !== "all" ? ` Â· ${selectedRankLabel}` : ""}{query ? " found" : ""}</span></footer>
    </section>
  </div>;
}
